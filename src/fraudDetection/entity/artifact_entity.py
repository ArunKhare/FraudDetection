"""Module for defining data classes for various artifacts in the project.

This module includes classes that represent different stages of a machine learning
pipeline, from data ingestion to model deployment. Each class is designed to encapsulate
the results and metadata of each stage.

Classes:
    DataIngestionArtifact: Represents the result of data ingestion.
    DataValidationArtifact: Represents the result of data validation.
    DataTransformationArtifact: Represents the result of data transformation.
    ModelTrainerArtifact: Represents the result of model training.
    ModelEvaluationArtifact: Represents the result of model evaluation.
    ModelPusherArtifacts: Represents the result of model pushing.
    Experiment: Represents an experiment with associated metadata.

Attributes:
    train_file_path (Path): Path to the training file.
    test_file_path (Path): Path to the test file.
    is_ingested (bool): Indicates whether data is ingested.
    message (str): Additional message related to the artifact.

Examples:
    >>> data_ingestion_artifact = DataIngestionArtifact(
            train_file_path=Path("/path/to/train.csv"),
            test_file_path=Path("/path/to/test.csv"),
            is_ingested=True,
            message="Data successfully ingested."
        )
    >>> data_validation_artifact = DataValidationArtifact(
            report_file_path=Path("/path/to/validation_report.txt"),
            schema_file_path=Path("/path/to/schema.json"),
            class_proportion_train=0.8,
            is_validated=True,
    ...
    >>> model_trainer_artifact = ModelTrainerArtifact(
            ...
            trained_model_file_path=Path("/path/to/trained_model.pkl"),
            train_accuracy=0.85,
            test_accuracy=0.82,
            train_f1_score=0.78,
            test_f1_score=0.75
