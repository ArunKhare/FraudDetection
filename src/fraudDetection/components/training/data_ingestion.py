    ...
    def download_transaction_data(self, ) -> str:
        try:
            ...
            if not os.path.exists(zip_filepath) or choice == 'Yes':
                ...
                tqdm(kaggle.api.dataset_download_files(dataset_name, zip_download_dir), mininterval=5,
    ...
    def unzipfile(self, zip_file_path) -> None:
        try:
            ...
            if size > 100:
                ...
                raise FraudDetectionException("File is almost empty", sys)
        except Exception as e:
            raise FraudDetectionException(e, sys) from e
            ...
            if is_dir_empty(train_file_path) or is_dir_empty(test_file_path):
                ...
                if self.data_ingestion_config.test_size > 1 or self.data_ingestion_config.test_size <= 0:
                    raise FraudDetectionException(
                        f"Invalid test_size: {self.data_ingestion_config.test_size}. Test size must be between 0 and1",
                        sys)
                ...
                strat_train_set, strat_test_set = train_test_split(df, test_size=self.data_ingestion_config.test_size,
                                                                   random_state=42,
