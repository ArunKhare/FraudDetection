...
from fraudDetection.entity import DataIngestionArtifact, DataTransformationConfig, DataValidationArtifact, \
...
def get_data_processing_objects() -> Pipeline:
    ...
    feature_extractor = Pipeline(steps=[
        ('feature_selection',
         SelectFromModel(ExtraTreesClassifier(n_estimators=50, max_features="log2"), threshold=0.01))
    ])
...
def processing_data(impute_sampler_obj, preprocessor_obj, train_X, train_y, test_X, test_y):
    # preprocessing train dataset
    resample_train_X, resample_train_y = impute_sampler_obj.fit_transform(train_X, train_y)
...
class DataTransformation:
    ...
    def initiate_data_transformation(self):
        try:
            ...
            train_arr_with_y, test_arr_with_y = processing_data(inputer_sampler_obj, preprocessor_obj,
            ...
            preprocessing_obj_file_path = Path(
                os.path.join(self.data_transformation_config.preprocessing_object_file_path))
            impute_sample_obj_file_path = Path(
                os.path.join(self.data_transformation_config.imputer_sampler_object_file_path))
