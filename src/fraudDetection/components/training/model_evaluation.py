""" This module evaluates the latest trained model with the model already in production """
import os
import sys
...
class ModelEvaluation:
    ...
    def get_best_model(self):
        """Gets the reference best model already in production or create a new reference to best model
        Returns:
    ...
    def update_evaluation_report(self, model_evaluation_artifact):
        """update the reference of best model in yaml file and archives the references
        Args:
    ...
    def initiate_model_evaluation(self):
        """implements the evaluation of models 'Trained and production'_, and model tracking in MLflow
        Returns:
            model_evaluation_artifact (obj:'ModelEvaluationArtifacts'): evaluation artifact with path to best model
        """
        try:
            ...
            if model is None:
                ...
                eval_file_path = (
                    self.model_evaluation_config.model_evaluation_file_path.parent
                )
                ...
                processor_file_path = (
                    self.data_transformation_artifact.processed_object_file_path
                )
                imputed_sampler_file_path = (
                    self.data_transformation_artifact.impute_sampler_object_file_path
                )
                ...
                temp_root = Path("temp")
                artifact_file_path1 = Path(
                    os.path.join(temp_root, "imputed_sampler_obj.pkl")
                )
                artifact_file_path2 = Path(
                    os.path.join(temp_root, "ser_preprocessing_obj.pkl")
                )
                ...
                if metric_info_artifact.model_index == 1:
                    ...
                    mlflow.sklearn.log_model(
                        ...
                        registered_model_name=str(
                            metric_info_artifact.model_object.__class__.__name__
                        ),
                        signature=signature,
                    ...
                    mlflow.log_metric(
                        "train_fbeta_score", metric_info_artifact.train_fbeta_score
                    )
                    mlflow.log_metric(
                        "test_fbeta_score", metric_info_artifact.test_fbeta_score
                    )
                    mlflow.log_metric(
                        "train_roc_auc_score", metric_info_artifact.train_roc_auc_score
                    )
                    mlflow.log_metric(
                        "test_roc_auc_score", metric_info_artifact.test_roc_auc_score
                    )
                    mlflow.log_metric(
                        "train_precision_score",
                        metric_info_artifact.train_precision_score,
                    )
                    mlflow.log_metric(
                        "test_precision_score",
                        metric_info_artifact.test_precision_score,
                    )
                    mlflow.log_metric(
                    ...
                    mlflow.log_metric(
                        ...
                        metric_info_artifact.train_accuracy_score,
                    )
                    mlflow.log_metric(
                        "test_accuracy_score", metric_info_artifact.test_accuracy_score
                    )
                ...
                ModelEvaluation._get_output_schema(eval_file_path, input_schema)
            return model_evaluation_artifact
