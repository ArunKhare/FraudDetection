...
class DataValidation:
    ...
    def get_train_test_df(self) -> tuple:
        ...
        try:
            ...
            num_files_train = len(
                [f for f in train_files if os.path.isfile(Path(os.path.join(train_file_dir, f))) if f.endswith('.csv')])
            num_files_test = len(
                [f for f in test_files if os.path.isfile(Path(os.path.join(test_file_dir, f))) if f.endswith('.csv')])
    ...
    def validate_dataset_scheme(self, train_df, test_df):
        ...
        try:
            ...
            mismatch_dtype_cols = [col for col, dtype in data_schema.items() if
                                   col in train_df.columns and train_df[col].dtype != dtype]
