...
class DataValidation:
    ...
    def __init__(
        ...
        """initialize the Configuration artifacts with paths to artifacts"""
        try:
    ...
    def get_train_test_df(self) -> tuple:
        ...
        try:
            ...
            logging.info(
                f"Is train file and test file list exist with first files as : {train_files[0]}, {test_files[0]}"
            )
            ...
            num_files_train = len(
                [
                    ...
                    if os.path.isfile(Path(os.path.join(train_file_dir, f)))
                    if f.endswith(".csv")
                ]
            )
            num_files_test = len(
                [
                    f
                    for f in test_files
                    if os.path.isfile(Path(os.path.join(test_file_dir, f)))
                    if f.endswith(".csv")
                ]
            )
            ...
            logging.info(
                f"train_df and test_df created. Total train_dir CSV files :{num_files_train} test_dir csv files {num_files_test}"
            )
    ...
    def validate_dataset_scheme(self, train_df, test_df):
        ...
        try:
            ...
            logging.info(
                f" train_df: {train_df.columns} {train_df.dtypes} \n test_df: {test_df.columns} {test_df.dtypes}"
            )
            ...
            mismatch_dtype_cols = [
                ...
                if col in train_df.columns and train_df[col].dtype != dtype
            ]
            ...
            if missing_cols:
                ...
                logging.info(
                    f"train test file schema are as per the Schema {data_schema}"
                )
    ...
    def check_class_balance(strat_train_set, strat_test_set, target_col) -> None:
        """static method checks for class imbalance in the target feature
        Args:
            strat_train_set (obj:'pd.DataFrame'): training dataset
            strat_test_set (obj:'pd.DataFrame'): testing dataset
            target_col (obj:'pd.Series'): target feature
        Returns:
            class_proportion_ratio (float): ration of class balance in target feature
        """
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        Returns:
            data_validation_artifact:(obj:'DataValidationArtifact): artifact having path the validation repots
        """
        try:
            train_df, test_df = self.get_train_test_df()
            is_data_schema_validated, target_col = self.validate_dataset_scheme(
                train_df, test_df
            )
            class_proportion_train = self.check_class_balance(
